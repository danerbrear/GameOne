...
public class GUI implements GameObject {
    ...
    public GUI(Sprite backgroundSprite, GUIButton[] buttons, int x, int y, boolean fixed){
        ...
        if (backgroundSprite != null) {
            rect.w = backgroundSprite.width;
            rect.h = backgroundSprite.height;
        }
    }
    ...
    public void render(RenderHandler renderHandler, int xZoom, int yZoom){
        if (backgroundSprite != null){
            renderHandler.renderSprite(backgroundSprite, rect.x, rect.y, xZoom, yZoom, fixed);
        }

        if (buttons != null){
            for (int i = 0; i < buttons.length; i++) {
                buttons[i].render(renderHandler, (int)xZoom, (int)yZoom, rect);
            }
        }
    }
